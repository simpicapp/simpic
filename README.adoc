...
Simpic has a Go backend and a JavaScript frontend that uses Vue.js. The Go code lives under `cmd` (for the program
entry points) and `internal` (for everything else). The JavaScript is contained under the `frontend` folder.
